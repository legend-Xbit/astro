...
/** Skill directories prefixed with `_` are authoring templates, not skills. */
const isTemplateDirectory = (name: string) => name.startsWith('_') || name.startsWith('.');

/** Astro's original runner hard-required three cases per skill; keep that as the default target. */
const minimumCases = Number(process.env.SKILL_EVAL_MIN_CASES ?? 3);

export function loadEvalCases(): EvalCase[] {
	const skillDirectories = readdirSync(skillsDirectory, { withFileTypes: true })
		.filter((entry) => entry.isDirectory() && !isTemplateDirectory(entry.name))
	...
	return skillDirectories.flatMap((skillDirectory) => {
		...
		if (!Array.isArray(manifest.evals) || manifest.evals.length < minimumCases) {
			throw new Error(`${manifestPath} must contain at least ${minimumCases} eval case(s)`);
		}
